  ...
  methods: {
    queryContacts () {
      this.$get(`/cos/chat-record/contacts/user/${this.currentUser.userId}`).then((r) => {
        this.contacts = []
        r.data.data.forEach(contact => {
          // this.contacts.push({
          //   id: contact.hotelUserId == this.currentUser.userId ? contact.userId : contact.hotelId,
          //   userId: contact.hotelUserId == this.currentUser.userId ? contact.hotelId : contact.userId,
          //   name: contact.hotelUserId == this.currentUser.userId ? contact.userName : contact.hotelName,
          //   avatar: (contact.hotelUserId == this.currentUser.userId) ? 'http://127.0.0.1:9527/imagesWeb/' + contact.userImages.split(',')[0] : 'http://127.0.0.1:9527/imagesWeb/' + contact.hotelImage.split(',')[0],
          ...
          this.contacts.push({
            ...
            avatar: 'http://127.0.0.1:9527/imagesWeb/' + contact.contactImage.split(',')[0],
            lastMessage: contact.lastMessage,
            status: 'online'
          })
        })
    ...
    selectContact (contact) {
      ...
      // 这里可以加载与该联系人的历史消息
      this.loadMessages(contact.id, contact.userId, contact.studentId, contact.hotelUserId)
    },
    loadMessages (hotelId, userId, studentId, hotelUserId) {
      let param = {}
      if (hotelUserId == this.currentUser.userId) {
        param = {
          userId: hotelId,
          hotelId: userId
        }
      ...
      this.$get(`/cos/chat-record/list`, param).then((r) => {
        // 将后端返回的聊天记录转换为前端需要的格式
        this.messages = r.data.data.map(message => ({
          id: message.id,
          sender: message.studentId == this.currentUser.userId ? 'me' : message.toUserId.toString(),
