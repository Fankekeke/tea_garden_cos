<template>
  <a-modal v-model="show" title="修改活动报名" @cancel="onClose" :width="800">
    ...
    <a-form :form="form" layout="vertical">
      <a-row :gutter="20">
        <a-col :span="12">
          <a-form-item label='活动报名标题' v-bind="formItemLayout">
            <a-input v-decorator="[
            'title',
        ...
        <a-col :span="24">
          <a-form-item label='活动报名内容' v-bind="formItemLayout">
  ...
  methods: {
    ...
    handleSubmit () {
      ...
      this.form.validateFields((err, values) => {
        ...
        if (!err) {
          this.loading = true
          this.$put('/cos/activity-participants', {
            ...values
          }).then((r) => {
            this.reset()
            this.$emit('success')
