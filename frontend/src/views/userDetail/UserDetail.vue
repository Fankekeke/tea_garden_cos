      ...
      <div class="user-profile-header">
        ...
        <div class="user-info">
          ...
          <div class="user-stats">
            ...
            <span class="stat-item" v-if="currentUser.userId !== userId" @click="startChat">
              <a-icon type="message" />
              在线沟通
            </span>
          </div>
...
export default {
  ...
  methods: {
    ...
    getUserIdFromRoute () {
      // 从路由参数中获取用户ID
      this.userId = this.$route.query.id || this.currentUser.userId
      this.queryContacts()
    },
    queryContacts () {
    ...
    collectUserCheck (deleteFlag) {
      this.$post(`/cos/focus-info`, {userId: this.userId, collectUserId: this.postDetail.userId, deleteFlag}).then((r) => {
        this.postReplyDetail(this.postDetail)
        this.$message.success(deleteFlag === 0 ? '关注成功！' : '取消关注成功！')
      })
    },
    collectPostCheck (deleteFlag) {
      this.$post(`/cos/collect-info`, {userId: this.userId, postId: this.postDetail.id, deleteFlag}).then((r) => {
        this.postReplyDetail(this.postDetail)
        this.$message.success(deleteFlag === 0 ? '收藏贴子成功！' : '取消收藏成功！')
      })
    },
    collectByUser (postId) {
      this.$get(`/cos/post-info/collcet`, {userId: this.userId, postId}).then((r) => {
    ...
    commit () {
      if (this.replyContent !== '') {
        let data = {userId: this.userId, content: this.replyContent, postId: this.postDetail.id, replyUserId: this.replyUser}
...
.stat-item {
  ...
  &.chat-button {
    ...
    &:hover {
      background: linear-gradient(90deg, #43A047, #1B5E2C);
    }
  }
}
</style>
